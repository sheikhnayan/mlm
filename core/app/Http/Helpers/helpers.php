...
{           
        ...
        $headers .= 'From: support@safeword1.com'."\r\n".
            'Reply-To: support@safeword1.com'."\r\n" .
  ...
  while ($id != "" || $id != "0") {
    if (isUserExists($id)) {
      ...
      if ($posUser -> plan_id != 0) {
        ...
        $left_total = User::where('pos_id', $posid) -> where('position', 1)-> where('plan_id','!=',0) -> get();
        $right_total = User::where('pos_id', $posid) -> where('position', 2) -> where('plan_id','!=',0) -> get();

        if ($position == 1) {
          if ($right_total -> count() > 0) {
        ...
        $first = $posUser -> pos_id;
        if ($first != 0) {
          $firstUser = User::where('id', $first) -> first();

          $posi = $posUser -> position;

          if ($posi == 1) {
            $firstUserRight = User::where('pos_id', $first) -> where('position', 2) -> first();
            if($firstUserRight != null){
            $check = User::where('pos_id', $firstUserRight -> id) -> where('plan_id','!=',0) ->  get();
            $count = $check -> count();
            }else{
                $count = 0;
            }
            ...
            if ($count > 1) {
              ...
              $firstUser -> total_binary_com += $amount;
              $firstUser -> save();
              ...
              $firstUser -> transactions() -> create([
                ...
                'remark' => 'binary_commission',
                'trx' => getTrx(),
                'post_balance' => getAmount($firstUser -> balance),
              ]);
            }
          } else {
          $firstUserLeft = User::where('pos_id', $first) -> where('position', 1) -> first();
          if($firstUserLeft != null){
            $check = User::where('pos_id', $firstUserLeft -> id) -> where('plan_id','!=',0) ->  get();
            $count = $check -> count();
            }else{
                $count = 0;
            }
...
function referralComission($user_id, $details, $req_amount)
{
...
function showSingleUserinTree($user)
{
    ...
    if ($user) {
        ...
        $img = getImage('assets/images/logoIcon/favicon.png','120x120');
