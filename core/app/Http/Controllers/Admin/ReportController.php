...
class ReportController extends Controller
{
    ...
    public function dailyCom(Request $request)
    {
        if ($request->userID)
        {
            $user = User::findOrFail($request->userID);
            $page_title = $user->username . ' - Plan Commission Logs';
            $transactions = Transaction::where('user_id', $user->id)->where('remark', 'plan_commission')->with('user')->latest()->paginate(getPaginate());
        }else {
            $page_title = 'Plan Commission Logs';
            $transactions = Transaction::where('remark', 'plan_commission')->with('user')->latest()->paginate(getPaginate());
        }
